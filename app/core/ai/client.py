...
class AIClient:
    ...
    def chat_full(
        ...
        **kwargs
        ...
        """
        调用 AI 模型，返回 content 与 thinking（reasoning_content）。
        thinking 仅当模型支持时非空（如 deepseek-reasoner）。
        """
        params = {
            "model": self.model,
            "messages": messages,
        ...
        if thinking:
            logger.info("[chat_full] Thinking received len=%d preview=%s", len(thinking), (thinking[:200] + "..." if len(thinking) > 200 else thinking))
        return {
    ...
    def chat_full_stream(
        ...
        **kwargs
        ...
        """
        流式调用，yield (content_delta, thinking_delta)。
        每块为 (content_chunk, thinking_chunk)，可能为空字符串。
        """
        params = {
            "model": self.model,
            "messages": messages,
    ...
    def validate_config(self) -> tuple[bool, str]:
        ...
        if not self.api_key:
            return False, "未配置 AI API Key，请在配置文件或环境变量 AI_API_KEY 中设置"
