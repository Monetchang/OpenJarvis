...
class AITranslator:
    ...
    def __init__(self, translation_config: Dict[str, Any], ai_config: Dict[str, Any]):
        ...
        self.system_prompt, self.user_prompt_template = self._load_prompt_template(
            translation_config.get("PROMPT_MODULE", "ai_translation_prompt")
        )

    def _load_prompt_template(self, module_name: str) -> tuple:
        """从 app.core.prompts 加载提示词模块"""
        try:
            mod = importlib.import_module(f"app.core.prompts.{module_name}")
            system = getattr(mod, "SYSTEM", "")
            user = getattr(mod, "USER_TEMPLATE", getattr(mod, "TEMPLATE", ""))
            return system, user
        except Exception:
            return self._get_default_prompt()

    def _get_default_prompt(self) -> tuple:
        """获取默认翻译提示词"""
        system_prompt = f"""You are a professional translator. 
Translate the given text to {self.target_language}.
Keep the original meaning and style.
For technical terms, maintain accuracy."""
